...
namespace Leopotam.EcsLite {
    ...
    public class EcsWorld {
        ...
        private EcsWorld(int poolDenseSize, int poolRecycledSize, Dictionary<Type, IEcsPool> poolHashes, Dictionary<int, EcsFilter> hashedFilters, List<EcsFilter> allFilters) {
            _poolDenseSize = poolDenseSize;
            _poolRecycledSize = poolRecycledSize;
            _poolHashes = new Dictionary<Type, IEcsPool>(poolHashes.Count);
            // Копирование словаря _poolHashes
            foreach (var kvp in poolHashes) {
                _poolHashes.Add(kvp.Key, kvp.Value.DeepCopy(this));
            }

            // Копирование словаря _hashedFilters
            _hashedFilters = new Dictionary<int, EcsFilter>(hashedFilters.Count);
            foreach (var kvp in hashedFilters) {
                _hashedFilters.Add(kvp.Key, kvp.Value.DeepCopy(this));
            }

            // Копирование списка _allFilters
            _allFilters = new List<EcsFilter>(allFilters.Count);
            foreach (var filter in allFilters) {
                _allFilters.Add(filter.DeepCopy(this));
            }
        }
        
        
         public EcsWorld DeepCopy() {
            EcsWorld newWorld = new EcsWorld(_poolDenseSize, _poolRecycledSize, _poolHashes, _hashedFilters, _allFilters);

            // Копирование значений примитивных типов
            newWorld._entities = (short[])_entities.Clone();
            newWorld._entitiesItemSize = _entitiesItemSize;
            newWorld._entitiesCount = _entitiesCount;
            newWorld._recycledEntities = (int[])_recycledEntities.Clone();
            newWorld._recycledEntitiesCount = _recycledEntitiesCount;
            newWorld._pools = (IEcsPool[])_pools.Clone();
            newWorld._poolsCount = _poolsCount;
            newWorld._masks = (Mask[])_masks.Clone();
            newWorld._masksCount = _masksCount;
            newWorld._destroyed = _destroyed;

            // Копирование массивов списков _filtersByIncludedComponents и _filtersByExcludedComponents
            newWorld._filtersByIncludedComponents = new List<EcsFilter>[_filtersByIncludedComponents.Length];
            newWorld._filtersByExcludedComponents = new List<EcsFilter>[_filtersByExcludedComponents.Length];
            for (int i = 0; i < _filtersByIncludedComponents.Length; i++) {
                if (_filtersByIncludedComponents[i] != null) {
                    newWorld._filtersByIncludedComponents[i] = new List<EcsFilter>(_filtersByIncludedComponents[i].Count);
                    foreach (var filter in _filtersByIncludedComponents[i]) {
                        newWorld._filtersByIncludedComponents[i].Add(filter.DeepCopy(newWorld));
                    }
                }

                if (_filtersByExcludedComponents[i] != null) {
                    newWorld._filtersByExcludedComponents[i] = new List<EcsFilter>(_filtersByExcludedComponents[i].Count);
                    foreach (var filter in _filtersByExcludedComponents[i]) {
                        newWorld._filtersByExcludedComponents[i].Add(filter.DeepCopy(newWorld));
                    }
                }
            }
        ...
        public void FastCopy(EcsWorld newWorld) {
            // Копирование значений примитивных типов
            for (var index = 0; index < _entities.Length; index++) {
                newWorld._entities[index] = _entities[index];
            }
            newWorld._entitiesItemSize = _entitiesItemSize;
            newWorld._entitiesCount = _entitiesCount;
            
            for (var index = 0; index < _recycledEntities.Length; index++) {
                newWorld._recycledEntities[index] = _recycledEntities[index];
            }
            newWorld._recycledEntitiesCount = _recycledEntitiesCount;
            
            for (var index = 0; index < _pools.Length; index++) {
                newWorld._pools[index] = _pools[index];
            }
            newWorld._poolsCount = _poolsCount;
            
            for (var index = 0; index < _masks.Length; index++) {
                newWorld._masks[index] = _masks[index];
            }
