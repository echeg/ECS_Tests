using System;
using Leopotam.EcsLite;
using System.Collections.Generic;
using System.Globalization;
using UnityEngine;
namespace EcsGenerator.LeoEcsLite2 {


class System93 : IEcsInitSystem, IEcsRunSystem{
 EcsWorld _world = null;
 EcsFilter _filter;
 public void Init (EcsSystems systems) {
  _world = systems.GetWorld ();
  _filter = _world.Filter<Component54>().Inc<Component356>().Inc<Component176>().End();
 }
 public void Run (EcsSystems systems) {
  foreach (int entity in _filter) {
  var q = 0;
   if (_world.GetPool<Component443>().Has(entity))
   {
    q+=1;
    var component1 = _world.GetPool<Component443>().Get(entity);
   }
   else
   {
    q-=1;
   }
   if (_world.GetPool<Component267>().Has(entity))
   {
    q+=1;
    var component1 = _world.GetPool<Component267>().Get(entity);
   }
   else
   {
    q-=1;
   }
   if (_world.GetPool<Component118>().Has(entity))
   {
    q+=1;
    var component1 = _world.GetPool<Component118>().Get(entity);
   }
   else
   {
    q-=1;
   }
   if (_world.GetPool<Component451>().Has(entity))
   {
    q+=1;
    var component1 = _world.GetPool<Component451>().Get(entity);
   }
   else
   {
    q-=1;
   }
  }
 }
}

}
